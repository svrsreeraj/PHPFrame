...
**************************************************************************************/
class vendorPaymentDetail extends modelclass
	{
		public function vendorPaymentDetailListing()
			{
				...
				$sql				=	"SELECT d.id as did,dp.id as pid,d.name,dp.total_comm_vendor AS total, 
										dp.used_comm_vendor AS used, dp.balance_comm_vendor AS balance,dp.date_updated,d.vendor_id
										FROM vod_deal as d,vod_deal_payments AS dp WHERE d.id=dp.deal_id  
										and (d.status='$cooldown' or d.status='$expired') and d.vendor_id >0 and d.vendor_id='$getData[vid]'
										$sqlCond ";
				...
				foreach($data as $key=>$val)
					{
						$pid					=	$val["pid"];
						$detailArr				=	$this->getdbcontents_sql("SELECT * FROM  `vod_deal_payments_trans` WHERE deal_payment_id='$pid' and vendor_id = '$getData[vid]' order by id desc");
		...
		public function vendorPaymentDetailBack()
			{
				$this->executeAction(false,"Listing",true,false,'','vid','vendorPayment.php');	
			}
		public function vendorPaymentDetailSearch()
			{
				$this->executeAction(false,"Listing",true);
			}
		public function vendorPaymentDetailReset()
			{
